            ...
            <div class="x_content">
				 ...
				 <div class="item form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">								
                                    	...
                                    	<?php if(isset($previous_financial_year_activate) && $previous_financial_year_activate>0 && $force_allow==0){?>
										   <input type="text" class="form-control date col-md-7 col-xs-12" id="date" name="date" required='required' autocomplete="on">
										<?php }else{ ?>
											<input type="text" class="form-control col-md-7 col-xs-12" id="date_new" name="date" value="<?php echo date('d-m-Y'); ?>" required='required' autocomplete="off" readonly>  
...
<?php 
	...
	elseif($allow_till_current_date && 1==0)
	{		?>
		$('.date').datepicker({
			format: date_format,
			startDate: '<?php print $voucher_start_date; ?>',
...
<?php } ?>
	...
	function checkifledgerSelected(selectedElem, ledger_id)
	{
		var currentElemID = $(selectedElem).attr('id');
		var dropdownID;
		var returnValue = false;
		$('.ledger_dropdown').each(function(i, obj) {
			dropdownID = $(obj).attr('id');
			console.log(dropdownID != currentElemID, $(obj).val() == ledger_id)
			if (dropdownID != currentElemID && $(obj).val() == ledger_id)
			{
				returnValue = true;
				return;
			}
	...
	$(document).on('change','.ledger_dropdown',function(event){
		if (checkifledgerSelected($(this), $(this).val()))
		{
			alert("Ledger is already selectec,Please select a different ledger");
			$(this).val('');
			$(this).trigger('change');
	...
	function add_particulars(){
		...
		html += "<td><select class='form-control select21 ledger_dropdown' name='pledger["+count+"]' id='ledger_id_"+count+"'><option value=''>--Select--</option>";
		<?php
		$options = "";
	...
	function add_head(){
		...
		html += "<td><select class='form-control select22' name='ledger_id["+count1+"]' id='ledger_id_"+count1+"'><option value=''>--Select--</option>";
		<?php  
		$options = "";
		foreach ($ledgers as $ledger) {  
			if($voucher->type_id== 1 && ($ledger->group_name == 'Cash-in-hand' ||  $ledger->base_id==5) ){
