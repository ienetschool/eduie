                                ...
                                <tbody>   
                                    <?php $count = 1; if(isset($itemissue) && !empty($itemissue)){ ?>
                                        <?php foreach($itemissue as $obj){
                                            $iDiscount =  $obj['mrp']- $obj['issue_price'];
                                            $iNetAmount = $obj['invoice_total'] + $obj['invoice_discount'];

                                            $iDiscountPercentage = ($obj['invoice_total']/$iNetAmount)*100;
                                            if ($iDiscountPercentage) $iDiscountPercentage = 100-$iDiscountPercentage;
                                            if ($obj['issue_price'] == $obj['mrp'] &&  $iDiscountPercentage >0 &&  $iDiscount <=0)
                                            {
                                                $iDiscount = round(($obj['mrp'])*($iDiscountPercentage/100),2);
                                                $obj['issue_price'] = $obj['mrp'] -  $iDiscount;
                                                if (isset($_GET['fix_discounts']) && $_GET['fix_discounts'] == 1)
                                                {
                                                    $this->itemissue->update('item_issue', array("issue_price"=>$obj['issue_price'], 'discount'=>$iDiscount), array('id' => $obj['id']));
                                                    // $this->db->query("");
                                        ...
                                        <tr>
                                            ...
                                            {
                                                if(has_permission(DELETE, 'inventory', 'issueitem')){ ?>
                               ...
                               <?php echo form_open_multipart(site_url('issueitem/add'), array('name' => 'add', 'id' => 'add', 'class'=>'form-horizontal form-label-left'), ''); ?>                                                                                              <?php $this->load->view('layout/school_list_form'); ?>     
								 ...
								 <div class="item form-group">                        
                                   ...
                                   <div class="col-md-6 col-sm-6 col-xs-12">
										<input type="text" class="form-control col-md-7 col-xs-12" id="date" name="issue_date" autocomplete="off" value="<?php echo date('Y-m-d') ?>" required="requires" readonly>										     
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <input type="text" class="form-control  col-md-7 col-xs-12" id="total" name="total" autocomplete="off"  readonly>										     
                                        <div class="help-block"><?php echo form_error('issue_date'); ?></div>
...
function confirmandRevert(invoice_id)
{
    if (confirm('Are you sure to revert the sale ?'))
    {
        ...
        if (reason)
        {
            location.href='<?php echo site_url('/issueitem/revert/'); ?>'+invoice_id+'?reason='+reason;
        }
    }
}
    $(document).ready(function() {
		...
		$('.date').datepicker({
          ...
          $('#datatable-responsive').DataTable( {
              ...
              buttons: [
                  ...
                  {
                        text: 'Update Item List Sale Data',
                        action: function ( e, dt, node, config ) {
                            location.href=base_url+"/issueitem/?fix_discounts=1";
                        }
                    }
              ],
  ...
  // Total over all pages
            ...
            $( api.column( 9).footer() ).html(
                discount_page +' ('+ total_discount +')'
            );
            $( api.column( 10 ).footer() ).html(
                sell_price_page +' ('+ total_sell_price +')'
            );
            $( api.column( 11).footer() ).html(
                quantity_page +' ('+ total_quantity +')'
            );
            ...
            "columnDefs": [
                        ...
                        {
                            "targets": [0,1,2,3,4,5,8,9, 10, 10,11,12,13] ,
                            "searchable": false,
                        },
                        {
                            "targets": [3,4] ,
...
$('#add_more_btn').hide();
    ...
    function calculate_total()
    {
        ...
        console.log( discount, $('.price_input'))
        ...
        $('.price_input').each(function(i, obj) {
             ...
             if(discount > 0)
            {
                var  discount_value =  ( price *( discount/100 ));
                price = price - discount_value;
            }
            console.log(price, discount_value, price)
    ...
    function calculate_grandtotal()
    {
       ...
       var grand_total = (total  +charges).toFixed(2)
    ...
    function populateItem(school_id) {
        ...
        function addItemGroup()
        {
            ...
            element += '<div class="col-md-6 col-sm-6 col-xs-12">';
            element += ' <div class=""> ';
            element += '  <input  name="mrp[]" placeholder=""  id="price_mrp_'+__item_id_no+'" data-id="'+__item_id_no+'"  value="0" type="text" class="form-control price_input" required="required" readonly/>';
            element += '</div>';
            element += '</div>';
            element += '</div>';
            ...
            element += '<div class="col-md-6 col-sm-6 col-xs-12">';
            element += ' <div class=""> ';
            element += '  <input  name="price[]" placeholder="" id="price_'+__item_id_no+'" data-id="'+__item_id_no+'"  value="0" type="text" class="form-control " required="required" readonly/>';
            element += '</div>';
            element += '</div>';
            element += '</div>';
