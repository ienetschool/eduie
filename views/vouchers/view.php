				...
				<table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0">                               
                                <thead> 
								 <tr>
                                        ...
                                        <th class="no-sort"><?php echo $this->lang->line('transaction_no'); ?></th>
										<th  width="10% !important" class="no-sort hidden-col">Invoice/Reciept No.</th>
										<th class="no-sort"> <?php echo $this->lang->line('head_ledger'); ?></th>
										<th class="no-sort"><?php echo $this->lang->line('dr_cr'); ?></th>
										<th class="no-sort"><?php echo $this->lang->line('total_amount'); ?></th>
										<th class="no-sort">Narration</th>
                                        <th class='no-print no-sort'><?php echo $this->lang->line('action'); ?></th>
                                    </tr>
									  </thead>
                                <tbody>   
                                    <?php $count = 1; if(isset($transactions) && !empty($transactions)){ ?>
                                        ...
                                        <tr<?php if($obj->cancelled != 0){ print " class='strikeRow'"; } else { print " class='dataRow'"; } ?>>
											...
											<td><a href="<?php echo site_url('transactions/view/'.$obj->id); ?>" target="_blank"><?php echo $obj->transaction_no; ?></a></td>
											<td>
												<?php echo isset($invoice_numbers[$obj->invoice_id]) ? '<a href="'.site_url('accounting/invoice/view/'.$obj->invoice_id).'" target="_blank">'.$invoice_numbers[$obj->invoice_id].'</a>' : (isset($reciept_numbers[$obj->donation_id]) ? '<a href="'.site_url('accounting/donation/view/'.$obj->donation_id).'" target="_blank" >'.$reciept_numbers[$obj->donation_id].'</a>' : (isset($salary_payments[$obj->salary_payment_id]) ? '<a href="'.site_url('payroll/payment/payslip/'.$obj->salary_payment_id).'" target="_blank" >'.$salary_payments[$obj->salary_payment_id].'</a>' : (isset($inventory_invoices[$obj->inventory_id]) ? '<a href="'.site_url($inventory_url[$obj->inventory_id].'/view/'.$obj->inventory_id).'" target="_blank" >'.$inventory_invoices[$obj->inventory_id].'</a>' : ""))); ?>
											</td>
