...
<?php 
$aTotalCols = array('7'=>'mrp',8=>"discount", 9=>"purchase_p", 10=>"purchase_q", 11=>"purchase_t");
?>
<style>
.miplus {
    position: absolute;
                                ...
                                <tbody>   
                                    <?php $count = 1; if(isset($itemstocks) && !empty($itemstocks)){
                                        ...
                                        <?php foreach($itemstocks as $obj){ 
                                            ...
                                            $iDiscount =  $obj['discount'];
                                            $iNetAmount = $obj['grand_total'] + $obj['invoice_discount'];

                                            $iDiscountPercentage = ($obj['invoice_total']/$iNetAmount)*100;
                                            ...
                                            {
                                                $iDiscount = round(($obj['mrp'])*($iDiscountPercentage/100),2);
                                                $obj['purchase_price'] = $obj['mrp'] -  $iDiscount;
                                                
                                                if (isset($_GET['fix_discounts']) && $_GET['fix_discounts'] == 1)
                                                {
                                                    $this->itemstock->update('item_stock', array("purchase_price"=>$obj['purchase_price'], 'discount'=>$iDiscount), array('id' => $obj['id']));
                                                    // $this->db->query("");
                                        ...
                                        <tr>
                                            ...
                                            <td>                                                 
                                                ...
                                                <?php } */?>
                                                 <?php 
                                                 if ( check_is_previous_year($obj['school_id']) != 1)
                                                 {
                                                    if(has_permission(DELETE, 'inventory', 'itemstock')){ ?>
                                                        <input type="button" value="Revert"  onclick="javascript: return confirmandRevert(<?php echo $obj['invoice_id']; ?>);" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i>
                                                    <?php } 
                                                 }
                                                    ?>  
                            ...
                            <div class="x_content"> 
                               ...
                               <div class="item form-group">                        
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <input type="text" class="form-control  col-md-7 col-xs-12" id="date" name="date" autocomplete="off" value="<?php echo date('Y-m-d') ?>"  readonly >										     
                            ...
                            <div class="item form-group">
                              ...
                              <!-- <div class="item form-group">                        
								...
								<div class="item form-group">                        
                                    ...
                                    <div class="item form-group accountfields">
                                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="name">Without Discount
                                        </label> 
                                        <div class="col-md-6 col-sm-6 col-xs-12">									
                                            <input type="checkbox" name="without_discount" id="without_discount" value="">					
                                        </div>									
									</div>									
                                ...
                                <div class="item form-group">                        
                                        ...
                                        <div class="col-md-6 col-sm-6 col-xs-12">
                                            <input type="text" class="form-control  col-md-7 col-xs-12" id="grand_total" name="grand_total" autocomplete="off"  readonly>										     
                                            <div class="help-block"><?php echo form_error('issue_date'); ?></div>
...
<link href="<?php echo VENDOR_URL; ?>datepicker/datepicker.css" rel="stylesheet">
 ...
 function confirmandRevert(invoice_id)
{
    if (confirm('Are you sure to revert the sale ?'))
    {
        let reason ;
         reason = prompt('Please enter a reason for revert or cancel');
        if (reason)
        {
            location.href='<?php echo site_url('/itemstock/revert/'); ?>'+invoice_id+'?reason='+reason;
        }
    }
}
	 var itemdropdown = '';
        ...
        <?php } ?>
          ...
          $('#datatable-responsive').DataTable( {
              ...
              buttons: [
                  ...
                  'colvis',
                  {
                        text: 'Update Item List Purchase Data',
                        action: function ( e, dt, node, config ) {
                            location.href=base_url+"/itemstock/?fix_discounts=1";
                        }
                    }
              ],
              "columnDefs": [
                    {
						"targets": [3,12,13, 14,15] ,
                        "visible": false,
					},
                     {
			 			"targets": [5,6] ,
                    ...
                    {
						"targets": [0,1,2,3,4,7,8,9, 10, 10,11,12,13] ,
                        "searchable": false,
					}
            ],
              search: true,              
...
<script type="text/javascript">
    ...
    $("document").ready(function() {
		...
		<?php } ?>
        $('#without_discount').change(function(){
            if ($(this).is(':checked'))
            {
                $('.discount_input').val(0)
                $('.discount_input').attr('readonly', 'true');
                $('.purchase_input').removeAttr('readonly');
                
            }
            else
            {
                $('.purchase_input').attr('readonly', 'true');
    ...
    $(document).on('keyup','.purchase_input', function(){ 
        calculate_total()
    })
    ...
    function calculate_total()
    {
        ...
        var without_discount = $('#without_discount').is(':checked');
        ...
        $('.price_input').each(function(i, obj) {
             ...
             $('#item_total_'+item_id).val(item_total);
    ...
    $('.fn_school_id').on('change', function(){      
       ...
       function get_itemstock_by_school(url){          
        ...
        function addItemGroup1()
        {
            ...
            element += '<div class="col-md-6 col-sm-6 col-xs-12">';
            element += ' <div class=""> ';
            element += '  <input  name="mrp[]" placeholder="" id="price_mrp_'+__item_id_no+'" data-id="'+__item_id_no+'"  type="text" class="form-control price_input " />';
            element += '</div>';
            element += '</div>';
            element += '</div>';
            ...
            element += '<div class="item form-group"> ';
            element += '    <label class="control-label col-md-3 col-sm-3 col-xs-12"  for="name"><?php echo $this->lang->line('discount'); ?></label>';
            element += '    <div class="col-md-6 col-sm-6 col-xs-12">';
            element += '        <div class=""> ';
            element += '        <input  name="discount[]" placeholder="" id="discount_'+__item_id_no+'" data-id="'+__item_id_no+'"  type="text" class="form-control discount_input " />';
            element += '        </div>';
            element += '    </div>';
            element += '</div>';
            ...
            element += '<div class="col-md-6 col-sm-6 col-xs-12">';
            element += ' <div class=""> ';
            element += '  <input  name="purchase_price[]" placeholder="" id="price_'+__item_id_no+'" data-id="'+__item_id_no+'"  type="text" class="form-control purchase_input" readonly/>';
            element += '</div>';
            element += '</div>';
            element += '</div>';
            ...
            element += ' <div class="item form-group"> ';
            element += '<label class="control-label col-md-3 col-sm-3 col-xs-12"  for="name">Item Total</label>';
            element += '<div class="col-md-6 col-sm-6 col-xs-12">';
            element += ' <div class=""> ';
            element += '  <input  name="item_total[]" placeholder="" id="item_total_'+__item_id_no+'" data-id="'+__item_id_no+'"  type="text" class="form-control " readonly/>';
            element += '</div>';
            element += '</div>';
            element += '</div>';
            ...
            $('#without_discount').trigger('change')
