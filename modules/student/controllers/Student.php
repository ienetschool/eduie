...
$transferselected='';
                               ...
                               else
                               {
                                ...
                                if($obj->status_type == 'passed')   $action.='<option value="passed" '.$passedselected.'>'. $this->lang->line('passed').'</option>';
...
exit;
    ...
    public function unalumni($student_id){
        ...
        {
            $school = $this->student->get_school_by_id($dropped_student->school_id);

            if($this->student->unalumni_student($student_id, $school_id, $school->academic_year_id))
            {
    ...
    public function  bulk_pass()
    {
        ...
        if ($_POST) {
            if(isset($_POST['student_ids']))
            {
                ...
                {
                    $this->student->bulk_pass_student($student_ids, $school_id, $school->academic_year_id);
    ...
    * ********************************************************** */
    ...
    public function dropStudent() {
        if($this->input->post('submit')) {
            ...
            $student = $this->student->get_single('students', array('id'=>$studentId));
            $school = $this->student->get_school_by_id($student->school_id);
            $update = $this->student->dropStudent($studentId, $dropDate, $dropReason, $student->school_id, $school->academic_year_id);
            redirect('student/index'); 
        }   
    }
    ...
    // ak
     ...
     public function  pass_student() {
        ...
        if($data) {
            ...
            $student = $this->student->get_single('students', array('id'=>$studentId));
            $school = $this->student->get_school_by_id($student->school_id);
            $update = $this->student->pass_student($studentId, $student->school_id, $school->academic_year_id);
            redirect('student/index'); 
        }   
    }
    ...
    public function transferStudent() {
        if($this->input->post('submit')) {
            ...
            $student = $this->student->get_single('students', array('id'=>$studentId));
            $school = $this->student->get_school_by_id($student->school_id);
            $update = $this->student->transferStudent($studentId, $transferDate, $transferReason,$student->school_id, $school->academic_year_id);
            //$this->index();
            redirect('student/index'); 
        }   
    }
