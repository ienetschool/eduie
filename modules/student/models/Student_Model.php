...
class Student_Model extends MY_Model {
    ...
    public function get_student_list($class_id = null, $school_id = null, $academic_year_id = null,$start=null,$limit=null,$search_text='',$orderby = null,$order_dir = "ASC"){
        ...
        $this->db->select('S.*, E.roll_no, E.class_id,E.academic_year_id, E.class_id, E.section_id, E.status_type as status_type'. $school_query );
        ...
        else
        {
            ...
            $this->db->or_where('E.academic_year_id in (select previous_academic_year_id from enrollments where student_id=S.id and school_id=S.school_id)');
            $this->db->group_end();
        }
...
public function get_students_results_by_sr($sr_numbers = array()){
    ...
    public function dropStudent($studentId, $dropDate, $dropReason, $school_id, $academic_year_id)
    {
    ...
    public function pass_student($studentId,  $school_id, $academic_year_id)
    {
    ...
    public function unalumni_student($studentId, $school_id, $academic_year_id)
    {
    ...
    public function bulk_pass_student($studentIds, $school_id, $academic_year_id)
    {
        // $this->db->set('status_type', 'passed');
        // $this->db->where('student_id', $studentId);
        // $this->db->where('academic_year_id', $academic_year_id);
        ...
        $this->db->set('status_type', 'passed');
        $this->db->where('student_id IN (' . implode(',', array_map('intval', $studentIds)) . ')');
        $this->db->where('academic_year_id', $academic_year_id);
        $this->db->update('enrollments');
        return true;
    }

    public function transferStudent($studentId, $transferDate, $transferReason, $school_id, $academic_year_id)
    {
    ...
    public function getAllAlumniStudentList($class_id = null, $school_id = null, $academic_year_id = null,$start=null,$limit=null, $search_text='',$orderby = null,$order_dir = "ASC"){
        ...
        $this->db->select("S.*, SC.school_name, E.roll_no, E.class_id, U.username, U.role_id,  C.name AS class_name, SE.name AS section_name,CONCAT(E.transfer_reason,E.drop_reason,S.reason_of_seperation) as alumni_reason
        ...
        // {
            // $this->db->where('E.status=2');
            // $this->db->where_in('E.status_type', ['drop','transfer','passed']);
        $this->db->group_start();

        $this->db->group_start();
        $this->db->where_in('E.status_type', ['drop','transfer']);
        if($academic_year_id){
            $this->db->where('E.academic_year_id', $academic_year_id); 
        }
        $this->db->group_end();
        ...
        $this->db->group_start();
        $this->db->or_where("E.status_type='passed'  and E.academic_year_id<$academic_year_id
        and (select count(*) from enrollments where  previous_academic_year_id=E.academic_year_id and student_id=S.id and school_id=S.school_id)=0");
        // $this->db->where('', $academic_year_id); 
        $this->db->group_end();
        ...
        // {
        //     $this->db->group_start();
        //     $this->db->where('E.academic_year_id not in (select previous_academic_year_id from enrollments where student_id=S.id and school_id=E.school_id)');
