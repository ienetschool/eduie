...
class payscalecategory_Model extends MY_Model {
	...
	public function insert_default($school_id){
		...
		foreach($cats as $l){
			...
			$this->db->select('AL.*');
			$this->db->from('account_ledgers AS AL');        
			$this->db->where('AL.name', $debit_ledger->name);		         		
			$this->db->where('AL.account_group_id', $debit_ledger->account_group_id);
			...
			$this->db->select('AL.*');
			$this->db->from('account_ledgers AS AL');        
			$this->db->where('AL.name', $credit_ledger->name);		         		
			$this->db->where('AL.account_group_id', $credit_ledger->account_group_id);
			$this->db->where('AL.school_id', $school_id);
