...
class Mark_Model extends MY_Model {
    ...
    public function get_student_list($school_id = null, $exam_id = null, $class_id = null, $section_id = null, $subject_id = null, $academic_year_id = null){
        ...
        $this->db->select('S.father_name, E.roll_no, E.class_id, E.section_id, C.name AS class_name, S.id AS student_id, S.name AS student_name, S.photo,  S.phone');
        $this->db->from('subjects AS SU');        
        $this->db->join('classes AS C', 'C.id = SU.class_id', 'left');
        $this->db->join('enrollments AS E', 'E.class_id = SU.class_id', 'left');        
