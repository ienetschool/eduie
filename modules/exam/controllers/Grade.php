...
class Grade extends MY_Controller {
    ...
    * ********************************************************** */
    private function _prepare_grade_validation() {
        ...
        $this->form_validation->set_rules('point', $this->lang->line('grade_point'), 'trim|callback_point');
    ...
    * ********************************************************** */
    public function name() {
        $school = $this->grade->get_school_by_id( $this->input->post('school_id'));
        if ($this->input->post('id') == '') {
            $grade = $this->grade->duplicate_check('name',  $this->input->post('school_id'), $this->input->post('name'), 0, $school->academic_year_id);
            if ($grade) {
            ...
            $grade = $this->grade->duplicate_check('name',  $this->input->post('school_id'), $this->input->post('name'), $this->input->post('id'), $school->academic_year_id);
            if ($grade) {
                $this->form_validation->set_message('name', $this->lang->line('already_exist'));
                return FALSE;
            } else {
