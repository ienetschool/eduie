...
$transferselected='';
                          ...
                          $action .= '<select  class="form-control col-md-7 col-xs-12 status-type"  name="status_type"  id="status_type" onchange="update_status_type('.$obj->id.', this.value);"><option value="regular" '.$selected.'>'.$this->lang->line('regular').'</option>';
						  
						  /********** to check the unpaid invoice ********************/
						  
									$invoices_unpaid = $this->student->get_invoice_list($obj->school_id, $obj->id);
									if(empty($invoices_unpaid))
									{
									$action .= '<option value="drop" '.$dropselected.'>'.$this->lang->line('drop').'</option>';
									}
		                     /******************** End of this code ***************/
							$tans = $this->get_due($obj->id);
						  if($tans<1)
						  {
