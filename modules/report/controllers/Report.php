...
class Report extends My_Controller {
    ...
    * @return          : null 
    * ********************************************************** */
    public function student_statics_old() {
        ...
        if ($_POST) {
            ...
            foreach($schools as $school_obj)
            {
                ...
                if(!empty( $academic_years))
                {
                    ...
                    // echo "<pre>"; var_dump($student_data[1482]); die(); 
    ...
    * ********************************************************** */
    public function student_statics() {
        ...
        // error_on();
        if ($_POST) {
            // $debugs  = $this->report->get_list('enrollments', array('student_id' => 241280));
            // echo "<pre>";
            ...
            if(!$school_id)
            {
                if($district_id)
                {
                    $schools = $this->report->get_school_list_new($district_id);
                    $this->data['district_id'] = $district_id;
                }
                else if($this->session->userdata('role_id') == SUPER_ADMIN){
                    $schools =  $this->report->get_school_list_new();
                }
                else if($this->session->userdata('dadmin') == 1){
                    $schools =  $this->report->get_school_list_new($this->session->userdata('district_id'));
                }
            }
            else
            {
            ...
            foreach($schools as $school_obj)
            {
                ...
                if($report_type != "class")
                {
                   $filter_id = $school_obj->$filter_column;
                   $filter_name = $school_obj->$filter_column_name;
                ...
                if(!empty( $academic_years))
                {
                    ...
                    $students_data = $this->report->get_student_data($school_id,$current_academic_year,$prev_academic_year,'class_id');
                        $class_order =array();
                        $iClassCount = 0;
                    ...
                    foreach($students_data as $obj)
                    {
                        ...
                        if(!isset($previous_year_boys_students[$class_id] )) $previous_year_boys_students[$class_id] = array();
                        if(!isset($current_year_boys_students[$class_id] )) $current_year_boys_students[$class_id] = array();
                        if(!isset($previous_year_girls_students[$class_id] )) $previous_year_girls_students[$class_id] = array();
                        if(!isset($current_year_girls_students[$class_id] )) $current_year_girls_students[$class_id] = array();
                        
                        if(!in_array($class_id, $class_order) ) {
                            $class_order[$iClassCount] = $class_id;
                            $iClassCount++;
                        }
                        if(!isset($school_data[$class_id]))
                        {
                            $school_data[$class_id] = array("filter_name" =>'',"prev"=>array(),"curr"=>array());
                            $school_data[$class_id]['filter_name'] = $obj->$filter_column_name;
                            $school_data[$class_id]['prev'] = $prev_data = array('boys'=>[],'girls'=>[],'total'=>[]);
                            $school_data[$class_id]['curr'] = $curr_data = array('boys'=>[],'girls'=>[],'total'=>[],"tc_boys"=>[],"tc_girls"=>[]);
                        }
                        else
                        {
                            $prev_data = $school_data[$class_id]['prev'];
                            $curr_data = $school_data[$class_id]['curr'];
                        }
                        if($obj->academic_year_id == $current_academic_year)
                        {
                            ...
                            if($obj->gender == "male" )
                            {
                                $current_year_boys_students[$class_id][] = $obj->student_id;
                                if ($obj->status_type != "regular" && !($obj->status_type == "passed" && $obj->promoted ==0)) {
                                    $curr_data['tc_boys'][] =  $obj->student_id;
                                }
                                $curr_data['boys'][] =  $obj->student_id;
                            ...
                            {
                                $current_year_girls_students[$class_id][] = $obj->student_id;
                                if ($obj->status_type != "regular" && !($obj->status_type == "passed" && $obj->promoted ==0)) {
                                    $curr_data['tc_girls'][] =  $obj->student_id;
                                }
                                $curr_data['girls'][] =  $obj->student_id;
                        ...
                        else  if($obj->academic_year_id ==  $prev_academic_year)
                        {
                            ...
                            $previous_year_students[$class_id][] = $obj->student_id;
                            if($obj->gender == "male" )
                            {
                                $previous_year_boys_students[$class_id][] = $obj->student_id;
                                $prev_data['boys'][] =   $obj->student_id;
                            }
                            else
                            {
                                $previous_year_girls_students[$class_id][] = $obj->student_id;
                                $prev_data['girls'][] =   $obj->student_id;
                            }
                        }
                        
                        $school_data[$class_id]['prev'] = $prev_data ;
                        $school_data[$class_id]['curr'] = $curr_data ;

                    }
                    $curr_year_boys = 0;
                    $curr_year_girls = 0;
                    $tc_boys =  $iPromotedTotal = $last_year_boys = $last_year_girls =  0;
                    $tc_girls = 0;
                    $iCount = 0;
                    $aPromotedTotal = array();
                    // var_dump(count($school_data));
                    if (count($school_data) == 0)
                    {
                        continue;
                    }
                    foreach($school_data as $key => $obj)
                    {
                        if($obj['filter_name'] == '')
                        {
                           //continue;
                        ...
                        $new_boys = array_diff($curr_all_boys,$prev_boys);
                        ...
                        $left_girls =  $tc_girls_data;//array_diff($prev_girls,$curr_girls);
                         ...
                         $current_year_girls_students_data =  $current_year_girls_students[$key];
                            if($iPreviousClassID != "") {
                                $previous_year_boys_students_data =  $school_data[$iPreviousClassID];
                                $previous_year_girls_students_data =  $school_data[$iPreviousClassID];
                        ...
                        $new_boys = array_diff($curr_all_boys,$prev_boys_compare);
                        $new_girls = array_diff($curr_all_girls,$prev_girls_compare);
                        ...
                        $aPromotedTotal = array_merge($aPromotedTotal , $curr_all_girls);
                        
                        
                        // echo "<pre>"; var_dump(  $aPromotedTotal); 
                        
                        $iPromotedBoys = count($aPromotedBoys);
                        $iPromotedGirls = count($aPromotedGirls);
                        ...
                        $iPromotedTotal = $iPromotedTotal +  $iPromoted;
                         ...
                         $iTotalBoys = $new_boys_count+$iPromotedBoys-$left_boys_count ;
                         $iTotalGirls = $new_girls_count+$iPromotedGirls -$left_girls_count ;

                        
                        if(!isset($student_data[$filter_id]))
                        {
                            $student_data[$filter_id] = array('filter_name'=>  $filter_name,'last_year_boys'=>$prev_boys_count,'last_year_girls'=>$prev_girls_count,'promoted_girls'=>$iPromotedGirls, 'promoted_boys'=>$iPromotedBoys,
                                'new_boys'=>$new_boys_count,'new_girls'=>$new_girls_count,  'alumni_boys'=>$left_boys_count, 'alumni_girls'=>$left_girls_count);
                        }
                        else
                        {
                            $student_data[$filter_id]['last_year_boys'] = $student_data[$filter_id]['last_year_boys'] + $prev_boys_count;
                            $student_data[$filter_id]['last_year_girls'] = $student_data[$filter_id]['last_year_girls'] + $prev_girls_count;
                            $student_data[$filter_id]['promoted_girls'] = $student_data[$filter_id]['promoted_girls'] + $iPromotedGirls;
                            $student_data[$filter_id]['promoted_boys'] = $student_data[$filter_id]['promoted_boys'] + $iPromotedBoys;
                            $student_data[$filter_id]['new_boys'] = $student_data[$filter_id]['new_boys'] + $new_boys_count;
                            $student_data[$filter_id]['new_girls'] = $student_data[$filter_id]['new_girls'] + $new_girls_count;
                            $student_data[$filter_id]['alumni_boys'] = $student_data[$filter_id]['alumni_boys'] + $left_boys_count;
                            $student_data[$filter_id]['alumni_girls'] = $student_data[$filter_id]['alumni_girls'] + $left_girls_count;
            ...
            $this->data['filter_title'] =  $sFilterTitle;
        $this->data['report_url'] = site_url('report/balance');
        $this->layout->title( 'Student statics | ' . SMS);
        $this->layout->view('student/statics_new_dev', $this->data);
    }
    ...
    * ********************************************************** */
    public function tattendance() {
        ...
        if ($_POST) {
            ...
            $this->data['teachers'] = $this->report->get_list('teachers', array('alumni'=> 0, 'status' => 1, 'school_id'=>$school_id));
            $session = $this->report->get_single('academic_years', array('id' => $academic_year_id, 'school_id'=>$school_id));
            $this->data['school'] = $this->report->get_school_by_id($school_id);
    ...
    * ********************************************************** */
    public function eattendance() {
        ...
        if ($_POST) {
            ...
            $this->data['employees'] = $this->report->get_list('employees', array('alumni' => 0, 'status' => 1, 'school_id'=>$school_id));            
            $session = $this->report->get_single('academic_years', array('id' => $academic_year_id, 'school_id'=>$school_id));
            $this->data['school'] = $this->report->get_school_by_id($school_id);
...
$school_id=null;
    ...
    * ********************************************************** */
    ...
    public function fee_report($debugmode = "") {
        error_on();
        check_permission(VIEW,1);
        $this->data['currrent_year_data'] = array();
        $this->data['previous_year_data'] = array();
            ...
            switch($report_type )
            {
                ...
                case "school":
                   ...
                   // $school = $this->report->get_school_by_id($school_id);
                    ...
                    { 
                        ...
                        if ($income_head->head_type == "transport")
                        {
                            ...
                            if($academic_year_id_filter && isset($yearly_stop_fare[$academic_year_id_filter]))
                                ...
                                if($section_data->class_id == 24)
                                {
                                    if (  $section_data->id == 744)$iCheckAmount = 0;
                                    $iCheckAmount = $iCheckAmount +  $iFeeAmount* $iStudentCount;;
                                }
                           
                            $iTotalFeeAmount = $total_fee_amount =  $iFeeAmount* $iStudentCount;
                            if (!isset($students_data[$section_data->class_id."".$section_data->id] ))
                            {
                                $students_data[$section_data->class_id."".$section_data->id] =  array( "section_id"  =>  $section_data->id , "class_id" =>  $section_data->class_id,"class_name"  =>  $section_data->class_name , "section_name" =>  $section_data->section_name, "fee_amount" => $iTotalFeeAmount,'total_fees_amount'=>0) ;
                            }
                            else
                                $students_data[$section_data->class_id."".$section_data->id]['fee_amount'] =  $students_data[$section_data->class_id."".$section_data->id]['fee_amount']+ $iTotalFeeAmount;
                        ...
                        else if ($income_head->head_type == "hostel")
                        {
                            $iStudentCount =  $section_data->member_count; 
                    ...
                    $students_fee_datas =  $this->report->get_section_fee_amount( $section_ids,$class_ids,$school_id,$income_head_id, $academic_year_id_filter, $income_head->head_type );
        ...
        $this->layout->view('student/fee_report', $this->data);
    ...
    public function installment_wise_new() {
        error_on();
        // check_permission(VIEW,1);
        ...
        if ($_POST) {
           ...
           foreach ($schools as $school)
           {
                   ...
                   // $school = $this->report->get_school_by_id($school_id);
                    ...
                    // if($income_head->head_type == "transport")
                    // {
                    //     $students_raw =  $this->report->get_transport_students( $school_id ,null  ,$academic_year_id_filter, null, $route_id, $section_id);
                    // }
                    // elseif($income_head->head_type == "hostel")
                    // {
                    //     $students_raw =  $this->report->get_hostel_students( $school_id ,null  ,$academic_year_id_filter,null, $hostel_id, $section_id);
                    // }
                 ...
                 // Assign the value of $hostel_id to $extra_value
                    ...
                    if($income_head->head_type == "transport")
                    {
                        $students_raw =  $this->report->get_transport_students( $school_id ,$class_id  ,$academic_year_id_filter,null, $route_id, $section_id);;
                    }
                    elseif($income_head->head_type == "hostel")
                    {
                        $students_raw =  $this->report->get_hostel_students( $school_id ,$class_id  ,$academic_year_id_filter,null, $hostel_id, $section_id);;
                    }
                    else
                    {
                      
                        $students_raw =  $this->report->get_students( $school_id,$class_id ,$academic_year_id_filter, null, null, $section_id, $income_head_id);
                    }
                    ...
                    if(!empty($students_raw))
                    {
                        ...
                        foreach($students_paid_datas as $students_paid_data)
                        {
                            ...
                            // $paid_amount =  $paid_amount+ $students_paid_data->net_amount;
                            // $net_amount =  $net_amount+ $students_paid_data->net_amount;
                            // $students_emi_data[$students_paid_data->class_id][$students_paid_data->academic_year_id][$students_paid_data->emi_type]  =$students_emi_data[$students_paid_data->class_id][$students_paid_data->academic_year_id][$students_paid_data->emi_type] ?? 0;
                            // $students_emi_data[$students_paid_data->class_id][$students_paid_data->academic_year_id][$students_paid_data->emi_type] = $students_emi_data[$students_paid_data->class_id][$students_paid_data->academic_year_id][$students_paid_data->emi_type] +  $paid_amount;
                            // $students_emi_data[$students_paid_data->class_id][$students_paid_data->academic_year_id][$students_paid_data->emi_type] =  $paid_amount;
                            
                            // $students_paid[$students_paid_data->class_id][$students_paid_data->academic_year_id] = $students_paid[$students_paid_data->class_id][$students_paid_data->academic_year_id] ?? 0;
                            // $students_paid[$students_paid_data->class_id][$students_paid_data->academic_year_id] =  $students_paid[$students_paid_data->class_id][$students_paid_data->academic_year_id] + $net_amount;
                            // // $students_paid[$students_paid_data->student_id][$students_paid_data->academic_year_id] = $net_amount;
                            // $students_discount[$students_paid_data->class_id][$students_paid_data->academic_year_id] = $students_discount[$students_paid_data->class_id][$students_paid_data->academic_year_id] ?? 0;
                            // $students_discount[$students_paid_data->class_id][$students_paid_data->academic_year_id] =  $students_discount[$students_paid_data->class_id][$students_paid_data->academic_year_id] + $discount_amount;
                            // $students_discount[$students_paid_data->student_id][$students_paid_data->academic_year_id] =  $discount_amount ;
                        }
                        ...
                        foreach( $students_raw as $student)
                        {
                            $class_data[$student->class_id] = $student->class;
                            if ($income_head->head_type == "transport")
                            {
                            ...
                            $student->emi = array();
                            $fee_data[$student->class_id][$student->academic_year_id]['emi'] =  $fee_data[$student->class_id][$student->academic_year_id]['emi'] ?? array();
                            if(strtolower($student->rte) == "yes" && (!isset($student->type) || (isset($student->type) && $student->type != "hostel" && $student->type != "transport")))
                                $total_paid_amount =    $student->fee_amount;

                            else
                                $total_paid_amount =    $student->total_paid  +  $student->total_discount ;
                             

                            foreach($emi_fees_row as $obj)
                            {
                                    $emi_fees_id = $obj->id;
                                // if (isset( $students_emi_data[$student->id][$student->academic_year_id][$emi_fees_id]))
                                // {
                                    ...
                                    {
                                       $emi_amount = ( $student->fee_amount*($obj->emi_per/100));
                                    }
                                    if(  $total_paid_amount  <= 0)
                                    {
                                       $emi_due  =  $emi_amount;
                                    }
                                    else if(  $total_paid_amount  >= $emi_amount)
                                    {
                                        $total_paid_amount  =  $total_paid_amount  - $emi_amount;
                                       $emi_due  =  0;
                                    }
                                    else
                                    {
                                        $emi_due  =  $emi_amount- $total_paid_amount ;
                                    ...
                                    $fee_data[$student->class_id][$student->academic_year_id]['emi'][$emi_fees_id]  =  $fee_data[$student->class_id][$student->academic_year_id]['emi'][$emi_fees_id]  ?? 0;
                                    $fee_data[$student->class_id][$student->academic_year_id]['emi'][$emi_fees_id]  =  $fee_data[$student->class_id][$student->academic_year_id]['emi'][$emi_fees_id]  + $emi_due;
                                // }
                            }
                            ...
                            $fee_data[$student->class_id][$student->academic_year_id]['student_count'] =  $fee_data[$student->class_id][$student->academic_year_id]['student_count']  ?? 0;
                            $fee_data[$student->class_id][$student->academic_year_id]['student_count']++;
                            $fee_data[$student->class_id][$student->academic_year_id]  = $fee_data[$student->class_id][$student->academic_year_id]  ?? array();
                            $fee_data[$student->class_id][$student->academic_year_id]['due_amount'] =  $fee_data[$student->class_id][$student->academic_year_id]['due_amount'] ?? 0;
                            $fee_data[$student->class_id][$student->academic_year_id]['fee_amount'] =  $fee_data[$student->class_id][$student->academic_year_id]['fee_amount'] ?? 0;
                            $fee_data[$student->class_id][$student->academic_year_id]['total_paid'] =  $fee_data[$student->class_id][$student->academic_year_id]['total_paid'] ?? 0;
                            $fee_data[$student->class_id][$student->academic_year_id]['total_discount'] =  $fee_data[$student->class_id][$student->academic_year_id]['total_discount'] ?? 0;

                            $fee_data[$student->class_id][$student->academic_year_id]['fee_amount'] =  $fee_data[$student->class_id][$student->academic_year_id]['fee_amount'] + $student->fee_amount;
                            $fee_data[$student->class_id][$student->academic_year_id]['total_paid'] =  $fee_data[$student->class_id][$student->academic_year_id]['total_paid'] + $student->total_paid;
                            $fee_data[$student->class_id][$student->academic_year_id]['total_discount'] =  $fee_data[$student->class_id][$student->academic_year_id]['total_discount'] + $student->total_discount;
                            $fee_data[$student->class_id][$student->academic_year_id]['due_amount'] =  $fee_data[$student->class_id][$student->academic_year_id]['due_amount'] + $student->due_amount;

                            // $fee_data[$student->class_id][$student->academic_year_id] = $student;
                        }
                    }
                    // die(); 
                    $this->data['fee_data']  = $fee_data;
        ...
        $this->layout->view('student/installment_new', $this->data);
           ...
           if($school_id)
           {
                 ...
                 // Assign the value of $hostel_id to $extra_value
                    ...
                    else
                    {
                        ...
                        $students_raw =  $this->report->get_students( $school_id,$class_id ,$academic_year_id_filter,$fees->fee_amount, null, $section_id);
                    }
