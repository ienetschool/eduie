...
class Report_Model extends MY_Model {
    ...
    public function get_section_data($filter_coloumn_filter = null,$filter_coloumn_value = null)
    {
        ...
        $this->db->where('E.status_type', 'regular'); 
    ...
    public function get_sections($school_id,$income_head_id , $type = "fee", $academic_year_id =0 )
    {
        ...
        {
            $this->db->select("S.class_id,S.id,S.name as section_name,C.name as class_name, RS.stop_fare as fee_amount, RS.yearly_stop_fare, count(E.id) member_count");
            $this->db->from('enrollments AS E');
            $this->db->join('classes AS C', 'C.id = E.class_id', 'left');
            $this->db->join('sections AS S', 'C.id = S.class_id and E.section_id=S.id','left');
            ...
            $this->db->group_start();
            $this->db->where('E.status_type', 'regular');
            $this->db->or_where("E.status_type='passed'  and (select count(*) from enrollments where  previous_academic_year_id=E.academic_year_id and student_id=S.id and school_id=S.school_id)=0");
            $this->db->group_end();
            $this->db->where("C.status",1 ); 
    ...
    public function get_section_paid_students_count2($section_ids,$class_ids,$school_id = "",$income_head_id)
    {
        ...
        $this->db->where("I.net_amount>0"); 
        $this->db->where("I.income_head_id = $income_head_id)" ); 
        $this->db->where(" I.student_id not in (select id from students where school_id=$school_id and (lower(rte)='yes'))"); 
    ...
    public function get_section_fee_amount($section_ids,$class_ids,$school_id = "",$income_head_id, $academic_year_id =null, $fee_type = "" )
    {
    ...
    public function get_student_data($school_id ="",$academic_year_id,$prev_academic_year_id =null,$order_by,$faculty = null, $regular = 0)
    {
        ...
        $this->db->select("E.*,S.gender,SC.school_name as school_name,SC.district_id,SC.zone_id,C.disciplines as faculty_id,S.caste, S.rte
        ...
        $this->db->order_by('E.academic_year_id, E.student_id');   
        $result =  $this->db->get();  
//         if($school_id ==1482)
//         {
//   echo $this->db->last_query();
...
//         }
    ...
    public function get_faculties($school_id = null,$zone_id =null, $district_id =null,$filter_coloumn = null)
    {
     ...
     public function get_hostel_students($school_id = null,$class_id =null, $academic_year_id =null, $previous_academic_year_id = null, $hostel_id = null, $section_id = null)
     {
         $this->db->select("S.*, E.academic_year_id, E.class_id
        ...
        if ($previous_academic_year_id)
        {
            ...
            if($academic_year_id){
                $this->db->where("(HM.academic_year_id=$previous_academic_year_id or HM.academic_year_id=$academic_year_id)");
                $this->db->where('E.academic_year_id', $academic_year_id);
            }
        }
        else
        {
            if($academic_year_id){
                $this->db->where("(HM.academic_year_id=$academic_year_id)");
                $this->db->where('E.academic_year_id', $academic_year_id);
            }
        }
        $this->db->group_start();
   ...
   public function get_transport_section_students_count($section_ids,$class_ids,$school_id = "",$academic_year_id)
   {
     ...
     public function get_transport_students($school_id = null,$class_id =null, $academic_year_id =null, $previous_academic_year_id = null, $route_id = null, $section_id = null)
     {
         $this->db->select("S.*, RS.stop_fare as fee_amount, RS.yearly_stop_fare as yearly_fee_amount, 'transport' as type, E.academic_year_id, E.class_id
         ,(select name from classes where id=E.class_id) as class
         ,(select name from sections where id=E.section_id) as section");
         $this->db->from('students AS S');        
         $this->db->join('enrollments AS E', 'E.student_id = S.id');
        ...
        if ($previous_academic_year_id)
        {
            $this->db->where("(E.academic_year_id=$academic_year_id or E.academic_year_id=$previous_academic_year_id)");   
            if($academic_year_id){
                $this->db->where("(TM.academic_year_id=$previous_academic_year_id or TM.academic_year_id=$academic_year_id )");
                $this->db->where('E.academic_year_id', $academic_year_id);
            }
        }
        else
        {
            $this->db->where('E.academic_year_id', $academic_year_id);   
            if($academic_year_id){
                $this->db->where("(TM.academic_year_id=$academic_year_id)");
                $this->db->where('E.academic_year_id', $academic_year_id);
            }
        }
    ...
    public function get_students($school_id,$class_id, $academic_year_id, $fee_amount, $previous_academic_year_id = null, $section_id = null, $income_head_id = null)
    {
        $sFeeAmount =  $fee_amount ? " , '$fee_amount' as fee_amount " : ",(select fee_amount from fees_amount where class_id=E.class_id and income_head_id=$income_head_id) as fee_amount";
        $this->db->select("S.* ,  coalesce(E.rte,S.rte) as rte, E.academic_year_id, E.class_id
        ,(select name from classes where id=E.class_id) as class
        $sFeeAmount
         ,(select name from sections where id=E.section_id) as section");
        $this->db->from(' enrollments AS E');        
    ...
    {
        $this->db->select("I.net_amount,I.discount,I.emi_type,E.student_id,  ,E.class_id, E.academic_year_id");
        $this->db->from('invoices I');
        $this->db->join('enrollments AS E', 'E.student_id = I.student_id','left');
        $this->db->where("E.school_id",$school_id );
        $this->db->where('I.reverted !=1');     
    ...
    public function get_school_list_new($district_id=null) {
         $this->db->select('S.*
         ,(select name from districts where id=S.district_id) as district_name
         ,(select name from zone where id=S.zone_id) as zone_name
         ');
         $this->db->from('schools AS S');
         $this->db->where('S.status', 1);
        if( $this->session->userdata('dadmin')==1){
            
            if( $this->session->userdata('dadmin_school_ids'))
            {
                 $this->db->where_in('S.id', $this->session->userdata('dadmin_school_ids'));
            }
            else if( $this->session->userdata('zone_id'))
            {
                 $this->db->where('S.zone_id', $this->session->userdata('zone_id'));
            }
            else if( $this->session->userdata('district_id'))
            {
