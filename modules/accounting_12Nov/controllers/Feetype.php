...
class Feetype extends MY_Controller {
    ...
    public function index($school_id = null) {
        ...
        $this->data['feetypes'] = $this->feetype->get_fee_type($school_id,@$school->financial_year_id);  
...
public function add() {
        ...
        if ($_POST) {
            ...
            if($j<=100) {   
                ...
                // var_dump($data);
                // die();
                if(!empty($school) && $data['head_type'] == "fee") {
                   
                    $checkgeneralfee = $this->feetype->check_general_fee($data['school_id'],$data['financial_year_id']);
                    if($checkgeneralfee)
                    {
                        error("You already have a general fee for current financial year");
                        redirect('accounting/feetype/add');
                        exit;
                    }
                }
