...
class Notice extends MY_Controller {
    ...
    * ********************************************************** */
    public function _send_message_notification($message_data = null) {
        ...
        $school                     = $this->notice->get_school_by_id($message_data['school_id']);
        $users                      = $this->notice->get_notice_users($message_data['school_id'],$message_data['role_id']);
        if(!$message_data['role_id'])
        {
            $super_admins =  $this->notice->get_notice_users($message_data['school_id'],SUPER_ADMIN);
            $users        = array_merge($users,$super_admins);
        }
        $notice                     = $this->notice->get_single_notice($message_data['id']);  
        $data['academic_year_id']   = $school ->academic_year_id;

        $data['subject']            = "New ".$this->lang->line('notice');   
        $data['sender_id']          = logged_in_user_id();
        $data['sender_role_id']     = $this->session->userdata('role_id');
        foreach ($users as $obj) {
           

            if($obj->id != '' && logged_in_user_id()!= $obj->id){                    
