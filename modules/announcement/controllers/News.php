...
class News extends MY_Controller {
    ...
    * ********************************************************** */
    private function _prepare_news_validation() {
        ...
        $this->form_validation->set_rules('image', $this->lang->line('image'), 'trim|callback_image|callback_image_size');   
    }
    ...
    * ********************************************************** */ 
    public function image_size() 
    {
        if ((!isset($_FILES['image'])) || $_FILES['image']['size'] == 0) {
            ...
            if(filesize($_FILES['image']['tmp_name']) >= 1048676) {
                $this->form_validation->set_message('image_size', 'The Image file size shoud not exceed 1MB!');
                return false;
            }
        }
        return true;
    ...
    * ********************************************************** */
      private function _upload_image(){
        ...
        if ($image != "") {
            if ($image_type == 'image/jpeg' || $image_type == 'image/pjpeg' ||
                ...
                if($converted_file = webpConverter($destination . $image_path))
                {
    ...
    public function _send_message_notification($message_data = null) {
        ...
        $school                     = $this->news->get_school_by_id($message_data['school_id']);
        $users                      = $this->news->get_news_users($message_data['school_id']);
       
        $news                     = $this->news->get_single_news($message_data['id']);  
        $data['academic_year_id']   = $school ->academic_year_id;

        $data['subject']            = "New ".$this->lang->line('news');   
        ...
        foreach ($users as $obj) {
            ...
            if($obj->id != '' ){                    
                ...
                $data['receiver_role_id'] = $obj->role_id;
