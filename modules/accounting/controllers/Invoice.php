               ...
               foreach($invoices as $obj){
                   ...
                   $action='';
                    ...
                    if (has_permission(DELETE, 'accounting', 'invoice')) { 
                     ...
                     if ( check_is_previous_year($school_id) != 1)
                     {
                        if (has_permission(DELETE, 'accounting', 'invoice')) { 
                            if ($obj->paid_status == 'paid') { 
                                $action.= '<a class="btn btn-danger btn-xs"  href="#"  onclick="javascript: return confirmandRevert('. $obj->id.');" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i>Revert</a>';
                            } 
                        }
                    }
...
private function _get_posted_inv_data()
    ...
    public function get_fee_amount()
    {
        ...
        if ($income_head->head_type == 'hostel') {
            ...
            $fee = $this->invoice->get_hostel_fee($student_id, null,$income_head->academic_year_id );
            $debug['query'] = $this->db->last_query();

            if (!empty($fee)) {
                $yearly_room_rent = $fee->yearly_room_rent ? json_decode($fee->yearly_room_rent,true) : array();
                $debug['yearly_room_rent'] =     $yearly_room_rent;
                $debug['academic_year'] = $academic_year->id;
    ...
    public function revert($id)
    {
        ...
        if($id)
        {
            ...
            $updated = $this->payment->update('account_transactions', array('cancelled'=>1, 'narration'=> $reason), array('invoice_id' => $id));
            success($this->lang->line('delete_success'));
        }
        else{
            error($this->lang->line('delete_failed'));
