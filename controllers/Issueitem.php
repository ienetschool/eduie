...
{	
    public function __construct()
    {
     ...
     public function index($school_id = null) {
        ...
        $financial_start_date =   date("Y-m-d",  strtotime($f_start));
		$financial_end_date = $f_end;

        $this->data['itemissue'] = $this->itemissue->get(null,$school_id, null, $voucher_id, $financial_start_date, $financial_end_date, $item_id );	
        ...
        $this->layout->view('issueitem/index_new', $this->data);            
       
    }
    public function revert($id){	
        $reason = $this->input->get('reason');								               
        $invoice = $this->itemissue->get_invoice($id);
        ...
        if($id)
        {
            $updated = $this->itemissue->update('item_issue', array('reverted'=>1), array('invoice_id' => $id));
            $updated = $this->itemissue->update('item_invoices', array('reverted'=>1, 'note'=> $reason ), array('id' => $id));
            $updated = $this->itemissue->update('account_transactions', array('cancelled'=>1), array('inventory_id' => $id));
