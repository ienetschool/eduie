...
{	
    public function __construct()
    {
     ...
     public function index($school_id = null) {
		...
		$this->data['itemstocks'] = $this->itemstock->get(null,$school_id, null,$financial_start_date, $financial_end_date, $item_id);
...
public function revert($id){	
	...
	{
		$invoice  = $this->itemstock->get_invoice($id);
		if ( check_is_previous_year($invoice->school_id) == 1)
		{
			error($this->lang->line('You can only revert from current year'));
			redirect('dashboard');        
		}
		$updated = $this->itemstock->update('item_stock', array('reverted'=>1), array('invoice_id' => $id));
		$updated = $this->itemstock->update('item_invoices', array('reverted'=>1, 'note'=>$reason), array('id' => $id));
		$updated = $this->itemstock->update('account_transactions', array('cancelled'=>1, 'narration'=> $reason), array('inventory_id' => $id));
		success($this->lang->line('delete_success'));
	}
	else{
		error($this->lang->line('delete_failed'));
				...
				if($school->default_voucher_Id_for_inventory > 0 || $stock_adjustment){	
				   ...
				   // echo "<pre>";
					...
					{			
						...
						{
								$transaction_id = 0;
								$voucher_id=$invoice_data['voucher_id'];
								...
								if($invoice_data['debit_ledger_id']>0 && $invoice_data['credit_ledger_id']>0){
									...
									$transaction['ledger_id']=$invoice_data['credit_ledger_id'];
									...
									$transaction['narration']= "Purchase Items - ";		   
									...
									if(isset($transaction['voucher_id'])){		
										...
										if ($transaction_id) {								
													...
													$detail['ledger_id']=$invoice_data['debit_ledger_id'];
													$edited_ledger_ids[] =  $invoice_data['debit_ledger_id'];

													$detail['amount']=($invoice_data['grand_total']-(float)$invoice_data['charges']);								
						...
						if($transaction_id || $stock_adjustment)
						{
							...
							foreach($data['item_id'] as $key => $value){
								$data1 = $data;
								$data1['item_id'] = $value;
								$data1['quantity'] =  $data['quantity'][$key] ?? 0;
								$data1['purchase_price'] = $data['purchase_price'][$key] ?? 0;
								$data1['discount'] = $data['discount'][$key] ?? 0;

								$data1['mrp'] = $data['mrp'][$key] ?? 0;
