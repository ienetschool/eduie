	...
	function create($voucher_id = "",$recipt='')
	{
		if ($_POST) {
           ...
           // $this->_prepare_transaction_validation();
			...
			$led_arr=array();
			$school = $this->transactions->get_school_by_id($voucher->school_id);
        ...
        $iForceAllow = $this->global_setting->force_new_entry ?? 0;
		...
		/*if(empty($voucher_last_entry)){
			...
			//if($f_start > date('Y-m-d',strtotime($startDate)) ){
				//$startDate=date('m/d/Y',strtotime($f_start));
		if(date('Y',strtotime($f_start))== 2024)
		{
		...
		if ($iForceAllow  == 2)
		{
			$this->data['voucher_start_date']= date("31-03-2024", strtotime($startDate));
			$this->data['voucher_end_date']= date("31-03-2024", strtotime($startDate));
		}
		else
		{
		...
		$this->data['force_allow'] = $iForceAllow;
    ...
    * ********************************************************** */ 
	...
	public function revert($id = null) {        
       ...
       // check_permission(DELETE);        
		...
		$transaction=$this->transactions->get_transactions_by_id($id);
		if ( check_is_previous_year($transaction->v_school_id) == 1)
		{
			error($this->lang->line('You can only rever transactio from current year'));
			redirect('dashboard');        
		}
		$ledger_ids = array($transaction->ledger_id);
		$transaction_details = $this->transactions->get_list('account_transaction_details', array('transaction_id' => $id));
		foreach ($transaction_details as $transaction_detail)
			$ledger_ids[] = $transaction_detail->ledger_id;
		// echo "<pre>"; var_dump($ledger_ids); die(); 
		
		$this->transactions->revert_transaction($id);
		
		$response = update_ledger_opening_balance($ledger_ids,$transaction->school_id);
		// die($response);
