...
{	
	...
	public function fix_income_statement_ob( $school_id = 0)
	{
		...
		if($school_id)
        {
			...
			$ledgers_list = $this->accountledgers->update_accountledger_opening_balance($school_id,$financial_year->id, array("1","2"));
			
			$this->accountledgers->update('schools', array('update_income_expense_opening_balance'=>0), array('id' => $school_id));
			success("Updated opening balance of ledgers");
...
//$this->form_validation->set_rules('budget_cr_dr', $this->lang->line('budget_cr_dr'), 'trim|required'); 		
	...
	public function view($id= null)
	{
		...
		{
			if($transaction_raw->invoice_id ?? false) {
				$invoice_ids[] = $transaction_raw->invoice_id;
			}
			if($transaction_raw->donation_id ?? false) {
				$reciept_ids[] = $transaction_raw->donation_id;
			}
			if($transaction_raw->salary_payment_id ?? false) {
				$payroll_ids[] = $transaction_raw->salary_payment_id;
			}
			if($transaction_raw->inventory_id ?? false) {
				$inventory_ids[] = $transaction_raw->inventory_id;
			}
