...
class Dashboard extends MY_Controller {
    ...
    public function index() {
        ...
        $school = $this->dashboard->get_school_by_id($school_id);

        $studentData = $this->dashboard->get_student_by_class($school_id, @$school->academic_year_id);
        $studentAttendenceData =  $this->dashboard->get_student_attendence_by_class($school_id, @$school->academic_year_id);
       
        $studentAttendence = array();
        foreach ($studentAttendenceData as $key => $value) {
            $total_student = $value->total_student;
            $studentAttendence[$value->class_id] = $total_student;
        }
        $this->data['studentData']   = $studentData;
        $this->data['studentAttendence']   = $studentAttendence;
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN && $this->session->userdata('dadmin') != 1){            
            ...
            // }
            // redirect("debugmode/show_all_pending_count");

            
            if(isset($school->update_income_expense_opening_balance) && $school->update_income_expense_opening_balance ==1 )
            {
                $financial_year=$this->ledgers->get_single("financial_years",array("id"=>$school->financial_year_id));
                if ($financial_year->previous_financial_year_id)
                {
                    redirect('debugmode/update_ledger_balaces'.'/'.$financial_year->previous_financial_year_id.'/'.$school_id);
                }                
            }
            if(isset($school->import_data) && $school->import_data ==1 )
            {
                // echo "<pre>"; var_.dump( $school); die(); 
    ...
    {
        if($this->session->userdata('role_id') == SUPER_ADMIN || 1==1){    
            if ($_POST) {
                $data = $this->_get_posted_message_data();
                $updated = $this->dashboard->update('global_setting', $data, array('status' => 1));
            ...
            $this->data['force_new_entry'] = $this->global_setting->force_new_entry ?? 0;
    ...
    private function create_financial_year($school_id, $iCurrentYearID ="") {
        ...
        if (!empty($check) && ($check->previous_financial_year_id)) 
        {
            if (!$check->is_running)
            {
                $previous['is_running'] = 1;
                $this->dashboard->update('financial_years', $previous, array('id' => $check->id));
            }
