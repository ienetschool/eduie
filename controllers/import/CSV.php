...
class CSV extends MY_Controller {
    ...
    private function _get_posted_data() {
		...
		else if($_POST['type']== 'student_new'){
			$status=$this->_student_new();
		}
		else if($_POST['type']== 'alumni_student'){
			$status=$this->_student();
		}
		else if($_POST['type']== 'final_result'){
			$status=$this->_final_result();
		}
		return $status;        
    } 
	...
	private function _final_result(){
		error_on();
		$file = $_FILES['bulk_data']['name'];
		$destination = $_SERVER['DOCUMENT_ROOT'].'\assets\csv\/'.$file;   
		$school_id  = $this->input->post('school_id');                     
		if($school_id && is_dir($_SERVER['DOCUMENT_ROOT'].'/assets/csv/'.$school_id))
		{
			$destination = $_SERVER['DOCUMENT_ROOT'].'/assets/csv/'.$school_id.'/'.$file;   
		}
        $final_array = [];
		$sr_numbers = [];
		...
		if (($handle = fopen($destination, "r")) !== FALSE) {
            ...
            while (($arr = fgetcsv($handle)) !== false) {	                
				...
				if ($arr[0] != '') {				                
					...
					$data['max_marks'] = $arr[1];
					$data['sr'] = $arr[0];
					$sr_numbers[] = $data['sr'] ;
					$data['obtain_mark'] = $arr[2];
					$data['grade'] = $arr[3];
					$data['percentage'] = $arr[4];
					$data['result'] = $arr[5];
					$final_array[$data['sr']] = $data;
                }
            }
        }
		$students_updated = [];
		...
		if(!empty($sr_numbers))
		{
			$students =$this->student->get_students_by_sr($sr_numbers,$school_id);
			foreach($students as $student)
			{
				$students_updated[$student->admission_no] = $student;
			}
			$student_results =$this->student->get_students_results_by_sr($sr_numbers);
			foreach($students as $student)
			{
				$students_updated[$student->admission_no] = $student;
			}
		...
		$update_array = [];
		$existing_result_ids = array_column($student_results,'student_id') ;
		foreach($final_array as $index => $final_data)
		{
			if(!isset($students_updated[$index]))
			{
				$iCountFaild ++;
			...
			$student = $students_updated[$index];
			$final_array[$index]['student_name'] = $student->name;
			$final_array[$index]['father_name'] = $student->father_name;
			$final_array[$index]['student_id'] = $student->id;
			if(in_array($student->id,$existing_result_ids))
			{
				$result_key = array_search ($student->id, $existing_result_ids);
				$student_result = $student_results[$result_key];
				$update_data =  $final_array[$index];
				$update_data['id'] = $student_result ['id'];
				$update_array[] =$update_data;
				unset($final_array[$index]);
			}
		}
		...
		{
			$inserted = $this->student->insert_batch('temp_result',$final_array);
		}
		if(!empty($update_array))
		{
			$updated = $this->student->update_batch('temp_result',$update_array,'id');
		}
		$created_count = count($final_array);
		$updated_count = count($update_array);

		$success_msg='';
		if($created_count > 0){
			$success_msg .= $created_count." data inserted successfully. ";
		}
		if($updated_count >0){
			$success_msg .= $updated_count. "  data Updated successfully..";
		}
		if($iCountFaild >0){
			$success_msg .= $iCountFaild. " Faild";
		}
		success($success_msg);
		unlink($destination);
        return TRUE;
...
//print_r($arr);					
	...
	private function _student_new(){
		...
		if (($handle = fopen($destination, "r")) !== FALSE) {
			...
			$admission_numbers = $existing_admission_nubers = array();
			while (($arr = fgetcsv($handle)) !== false) {	
				if(isset($arr[7]))
				{
					$admission_numbers[] = $arr[7];
				}
			}
			
			$admission_numbers_raw = $this->student->check_existing_students($school_id,$admission_numbers);
			
			$existing_admission_nubers = array_column($admission_numbers_raw,"admission_no");
            ...
            while (($arr = fgetcsv($handle)) !== false) {		
				...
				if ( isset($arr[1]) &&  isset($arr[5]) && isset($arr[7]) && $arr[1] != '' && $arr[5] != '' && $arr[7] != '' ) {					                				
					$data = array();
                    $enroll = array();
                    $user = array();
					$guardian_user=array();
					if(in_array( $arr[7],$existing_admission_nubers))
					{
						$duplicate_count++;	
						continue;
					}
...
// $date_str= $m."/".$d."/".$y;
						...
						$dob_formatted	= isset($arr[10]) ? date_format_converter($arr[10]) : "";
						$data['dob'] = $dob_formatted ? date('Y-m-d', strtotime($dob_formatted)) : '';						
						$data['gender'] = isset($arr[9]) ? strtolower($arr[9]) : '';
						if(strtolower($data['gender']) == "boy") $data['gender'] = "male";
						if(strtolower($data['gender']) == "girl") $data['gender'] = "female";
						
						$data['phone'] = '';
						$data['email'] ='';                					
						$data['religion'] = '';
						$data['caste'] = isset($arr[8]) ? $arr[8] : '';
						$data['second_language'] = isset($arr[6]) ? $arr[6] : '';
						$data['rural_urban'] =  '';
						$data['present_address']='';
					
						//$admission_date_formatted = $arr[11];
						$data['admission_date'] = date('Y-m-d');
						$data['admission_no']= isset($arr[7]) ? $arr[7] : '';
						$data['bpl'] =  '';
						$data['rte'] =  isset($arr[11]) ? $arr[11] : ''; ;
						$data['adhar_no'] =  isset($arr[4]) ? $arr[4] : ''; ;
						$data['physical_disability']=  '';
						$data['free_education']="";
						
						$data['previous_class']='';
						$data['previous_school']='';
						$data['previous_year_attended_days']='';
						$data['medium_of_instruction']=isset($arr[6]) ? $arr[6] : '';
						$data['health_condition'] =  '';
						$data['facilities_by_cwsn'] ='';
						$data['no_of_uniform_sets'] =  '';
						$data['free_text_books'] =  '';
						$data['free_transport'] =  '';
						$data['free_escort'] =   '';
						$data['mdm_benificiary'] =   '';
						$data['free_hostel'] =   '';
						$data['special_training'] =   '';
						$data['appeared_in_last_exam'] =   '';
						$data['last_exam_passed'] =   '';
						$data['last_exam_marks'] =   '';				
						$data['trade_sector'] =   '';
						$data['iron_folic_acid_tablets'] =   '';
						$data['deworming_tablets'] =   '';
						$data['vitamin_a_supplement'] =   '';										
						// add to guardian
						$guardian_user['school_id']=$school_id;
						$firstname_arr=explode(" ",$arr[2]);
						$firstname=strtolower($firstname_arr[0]);
						$username= $data['admission_no'];
						$guardian_user['username']=$school->school_code."-".$data['admission_no']."_P";
						$guardian_user['role_id']=GUARDIAN;
						...
						$firstname = strtolower($firstname_arr[0]);
						$username= $data['admission_no'];
						$user['username']=$school->school_code."-".$username;
						$user['role_id']=STUDENT;
						// first need to create user
						$data['user_id'] = $this->_create_user($user);
						// now need to create student
						...
						$class_name=$class;
						if((strtolower($class) == 'eleventh' || strtolower($class)== 'twelth') ){
								// make it arts
...
// $date_str= $d."/".$m."/".$y;
						...
						$firstname=strtolower($firstname_arr[0]);
						$username=$firstname.substr($arr[12],strlen($arr[12])-3)."_".substr(uniqid(),9);					
						$user['username']=$school->school_code."-".$data['admission_no'];
						$user['role_id']=STUDENT;
						// first need to create user
						$data['user_id'] = $this->_create_user($user);
						// now need to create student
...
$duplicate_count++;	
    ...
    private function _upload_file() {
        ...
        if ($file != "") {
			$destination = $_SERVER['DOCUMENT_ROOT'].'\assets\csv\/'.$file;   
			if($_POST['type']== 'final_result')
			{
				if($school_id && !is_dir($_SERVER['DOCUMENT_ROOT'].'/assets/csv/'.$school_id))
				{
					mkdir($_SERVER['DOCUMENT_ROOT'].'/assets/csv/'.$school_id) ;
				}
				if($school_id && is_dir($_SERVER['DOCUMENT_ROOT'].'/assets/csv/'.$school_id))
				{
					$destination = $_SERVER['DOCUMENT_ROOT'].'/assets/csv/'.$school_id.'/'.$file;   
	 ...
	 private function _insert_enrollment($enroll) {
        ...
        $data['roll_no'] = isset($enroll['roll_no']) && $enroll['roll_no'] ? $enroll['roll_no'] : "" ;
        $data['created_at'] = date('Y-m-d H:i:s');
