...
{	
    ...
    public function show_all_pending_count() {
        $result = $this->application->get_count_overview();
        echo "<pre>"; var_dump($result ); die(); 
        
    }
    public function disable_fy_import() {
        $this->application->update('schools',array("import_data"=>2), array("import_data"=>1));
    }
    public function enable_fy_import($all = 0) {
        if ($all)
        $this->application->update('schools',array("import_data"=>1), array("1"=>1));
        else
        $this->application->update('schools',array("import_data"=>1), array("import_data"=>2));

    }
    public function enable_single_fy($school_id) {
        $this->application->update('schools',array("import_data"=>1), array("id"=>$school_id));
    }
    ...
    public function create_new_ledgers($school_id)
    {
        ...
        $data['name'] = "Transportation Fee 24-25";
        $ledger = $this->application->get_single('account_ledgers', $data);
        if(empty( $ledger ))
        {
    ...
    public function disable_ayc_all($school_id) {

        $this->application->update('schools',array("create_academic_year"=>-1), array('create_academic_year'=>1));
        redirect("dashboard/index");
    }
    ...
    {	
        ...
        $this->db->last_query("update item_issue set issue_price=44.01, discount = 10.99 where id=56129");
        $classes = $this->db->query("select id, issue_price, quantity 
        ,(select grand_total from item_invoices where item_issue.reverted!=1 and id=invoice_id) as grand_total
        from item_issue where item_issue. reverted!=1 and issue_date>='2023-04-01' and issue_date<='2024-03-31' and item_id=3010 and invoice_id is not null  ")->result_array();
        echo "<table>";
        foreach ($classes as $sClass)
        {
            $sTotal = $sClass['issue_price'] * $sClass['quantity'];
            echo "<tr><td> $sClass[id] </td><td> $sClass[issue_price] </td><td>$sClass[quantity]</td><td>$sTotal</td><td>$sClass[grand_total]</td></tr>";
        }
        echo "</table>";
        $classes = $this->db->query("select grand_total from item_invoices where item_invoices.reverted!=1 and id in (select invoice_id from item_issue where reverted!=1 and  issue_date>='2023-04-01' and issue_date<='2024-03-31' and item_id=3010 and invoice_id is not null )")->result_array();

        echo "<table>";
        foreach ($classes as $sClass)
        {
            echo "<tr><td> $sClass[grand_total] </td></tr>";
        }
        echo "</table>";
    ...
    public function update_ledger_balaces($finacial_year_id = null,$school_id = null)
    {
        ...
        $agroup=$this->accountgroups->get_accountgroup_by_types($school_id, null);	
        $account_ledger_details = array();
           ...
           foreach($ledgers as $l){
               ...
               if(in_array($ag->type_id,array(4,3)))
               {
                  ...
                  if($ag->type_id == 3)
                  {
                        ...
                        $final_assets=    $final_assets+ $cb;		
                  }
                  else{
                    $final_liabilities= $final_liabilities + $cb;			
    ...
    function copy_ledger_balances_for_all($school_id = null,$previous_finacial_year_id = null)
    {
		 ...
		 foreach($other_transactions as $other_transaction)
		 {
           ...
           foreach($ledgers_list_updated as $ledger_id => $ledger){
               ...
               $ledger_transactions = isset($ledgers_transactions_updated[$ledger_id]) && $ledgers_transactions_updated[$ledger_id] ? $ledgers_transactions_updated[$ledger_id] :  array();
                   ...
                   if ($ledger->group_type == 3)
                   {
                        $final_assets=    $final_assets+ $ledgers[$ledger_id]->effective_balance;		

                   }
                   else if ($ledger->group_type == 4)
                   {
                     $final_liabilities =    $final_liabilities+ $ledgers[$ledger_id]->effective_balance;		
                   }
           ...
           $liability_difference =$asset_difference = 0;
        //    echo "<pre>"; var_dump($final_assets , $final_liabilities ); die(); 
        ...
        //     {
        //         $journal_ledger=$this->accountledgers->get_single('account_ledgers',array('school_id'=>$school->id,'lower(name)'=>'journel fund'));	
        //         echo "<pre>";
        //         if  (!empty( $journal_ledger))
        //         {
        //             $details =$this->accountgroups->get_single('account_ledger_details', array('financial_year_id'=> $current_financial_year_id , 'ledger_id'=>$journal_ledger->id));	
        
        //             if  (!empty( $details))
        //             {
        //                 $details_array =  array();
        //                 $details_array['ledger_id']=$journal_ledger->id;
        //                 $details_array['financial_year_id']=$current_financial_year_id;
        //                 $ledgers=$this->accountledgers->insert("account_ledger_details", $details_array);
        
        //             }
        //             else
        //             {
        //                 $ledgers=$this->accountledgers->update("account_ledger_details", array("opening_balance"=>$final_assets), array('id'=> $details->id));
        
        //             }
        //         }
        //     }
          
        //  var_dump($liability_difference); die(); 
        redirect("accountledgers/fix_income_statement_ob/$school_id");
    ...
    public function test_new()
    {
        ...
        // UPDATE `enrollments` SET `status_type` = 'passed'\nWHERE `id` IN (354147,354148)\nAND `academic_year_id` = '2317'
        $debugs  = $this->invoice->get_list('invoices', array('school_id'=>175, 'academic_year_id'=>2317));
        echo "<pre>";
        print_r($debugs);
