...
public function get_list($school_id = null) {
    ...
    public function getItemBySchool($school_id) {
        $this->db->select('item.id,item.name,item.item_category_id,item.unit,item_category.item_category,item_category.id as `item_category_id`');
        $this->db->from('item');
        $this->db->join('item_category', 'item_category.id = item.item_category_id');
