...
class Itemissue_model extends MY_Model {
    ...
    // }
    public function get($id = null,$school_id = null,$invoice_id = null, $voucher_id = null, $start_date = null , $end_date = null, $item_id =0) {

        $schoolselect = $school_id ? " " : " schools.school_name,";
        if ($id )
        {
            $this->db->select( $schoolselect .'item_issue.*,item.name as `item_name`,`item`.`item_code`,item.item_category_id,item_category.item_category, AT.transaction_no, AT.id as transaction_id,IN.invoice_no, IN.discount as invoice_discount, IN.grand_total as invoice_total, IN.id as invoice_id
        ...
        else
        {
            ...
            // ,AL1.name as debit_ledger
            // ,AL2.name as credit_ledger')->from('item_issue');
            $this->db->select( $schoolselect .'item_issue.*,item.name as `item_name`,`item`.`item_code`,item.item_category_id,item_category.item_category,IN.invoice_no, IN.discount as invoice_discount, IN.grand_total as invoice_total, IN.id as invoice_id
        ...
        $this->db->where('item_issue.reverted!=1');
        ...
        if($_GET['item_id_check'] ?? false)
        {
            $this->db->where('item_issue.item_id', $_GET['item_id_check']);
        }
        if($voucher_id)
        {
        ...
        if ( $item_id)
        {
            $this->db->where('item_issue.item_id', $item_id);
        }
        if($start_date && $end_date)
        {
            $this->db->where("DATE(item_issue.issue_date) BETWEEN '$start_date' AND '$end_date'");
        }
        // $this->db->limit(1);
         $query = $this->db->get();
         if($_GET['debug'] ?? false)
    ...
    {
        $this->db->select('I.*, AT.transaction_no
