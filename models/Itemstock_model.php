...
class Itemstock_model extends MY_Model {
    ...
    public function get($id = null,$school_id = null,$invoice_id = null, $start_date = null, $end_date = null, $item_id = null) {

        $this->db->select('schools.school_name,`item_stock`.*, `item`.`name`,`item`.`item_code`, `item`.`item_category_id`, `item`.`description` as des, `item_category`.`item_category`, `item_supplier`.`item_supplier`, `item_store`.`item_store`, AT.transaction_no,AT.id as transaction_id,IN.invoice_no,IN.grand_total,IN.discount as invoice_discount, IN.id invoice_id
        ...
        {
            $this->db->where("DATE(item_stock.date) BETWEEN '$start_date' AND '$end_date'");
        }
        if($invoice_id)
        {
        ...
        if ($id != null) {
         ...
         $this->db->where('item_stock.reverted!=1');

       if ($item_id)
       $this->db->where('item_stock.item_id', $item_id);
        $query = $this->db->get();
