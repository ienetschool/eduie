...
class Dashboard_Model extends MY_Model {
    ...
    public function get_student_by_class($school_id = null, $academic_year_id = null) {

        $this->db->select('COUNT(E.student_id) AS total_student, C.name AS class_name, C.id as class_id');
    ...
    public function get_student_attendence_by_class($school_id = null, $academic_year_id = null) {
        ...
        $this->db->from('enrollments AS E');
        $this->db->join('classes AS C', 'C.id = E.class_id', 'left');
        $this->db->join('student_attendances AS SA', 'SA.student_id = E.student_id', 'left');
        ...
        $this->db->where("SA.day_$iDay", 'P');
        $this->db->where('SA.month', $iMonth);
        $this->db->where('SA.year', $iYear);
        $this->db->where('E.status_type', 'regular');
        if ($school_id) {
            $this->db->where('E.school_id', $school_id);
        }
