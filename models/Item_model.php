...
class Item_model extends MY_Model {
 ...
 function get_item_list($school_id = null, $category_id =null, $group_id = null, $start_date = null , $end_date = null) {
		 $this->db->select('I.*, S.school_name,IC.item_category
         
         ,(select name from item_groups where id=IC.group_id) as group_name ');
    ...
    function get_item_list_dev($school_id = null, $category_id =null, $group_id = null, $start_date = null , $end_date = null) {
        $iYear = date('Y');
        $this->db->select("I.*, S.school_name,IC.item_category
        ,(select purchase_price from item_stock where item_id=I.id  and purchase_price>0 and reverted!=1  order by id desc limit 1) as last_purchase_value
        ,(select mrp from item_stock where item_id=I.id  and purchase_price>0 and reverted!=1  order by id desc limit 1) as mrp

        ,(select sum(quantity) from item_stock where item_id=I.id and date(date)<='$start_date'  and reverted!=1) as opening_count
        ,(select sum(purchase_price*quantity) from item_stock where item_id=I.id and date(date)<='$start_date'  and reverted!=1) as opening_value
        ,(select sum(quantity) from item_issue where reverted!=1 and item_id=I.id and date(issue_date)<='$start_date' and is_returned!=1) as opening_sale_count
        ,(select sum(issue_price*quantity) from item_issue where reverted!=1 and item_id=I.id and issue_date<='$start_date' and is_returned!=1) as opening_sale_value
        ,(select sum(quantity) from item_issue where reverted!=1 and item_id=I.id and return_date<='$start_date' and is_returned=1) as opening_returned
        ,(select sum(issue_price*quantity) from item_issue where reverted!=1 and item_id=I.id and return_date<='$start_date' and is_returned=1) as opening_returned_value
        ,(select sum(quantity) from item_stock where item_id=I.id and date>='$start_date' and date<='$end_date'  and reverted!=1) as purhcase_count_year
        ,(select sum(purchase_price*quantity) from item_stock where item_id=I.id and date>='$start_date' and date<='$end_date'  and reverted!=1) as purhcase_value_year
        ,(select sum(quantity) from item_issue where reverted!=1 and item_id=I.id and issue_date>='$start_date' and issue_date<='$end_date'  ) as sale_count_year
        ,(select sum(issue_price*quantity) from item_issue where reverted!=1 and item_id=I.id and issue_date>='$start_date' and issue_date<='$end_date' ) as sale_value_year
        ,(select sum(quantity) from item_issue where reverted!=1 and item_id=I.id and return_date>='$start_date' and return_date<='$end_date' and is_returned=1) as returned_year
        ,(select sum(issue_price*quantity) from item_issue where reverted!=1 and item_id=I.id and return_date>='$start_date' and return_date<='$end_date' and is_returned=1) as returned_year_valu
        ,(select name from item_groups where id=IC.group_id) as group_name ");
       $this->db->from('item AS I');       		
       $this->db->join('schools AS S', 'S.id = I.school_id', 'left');
       $this->db->join('item_category AS IC', 'IC.id = I.item_category_id', 'left');
   ...
   public function get_default_categories($school_id,$group_id,$real_grpup_id){
    ...
    public function getItemLastPrice($item_id = null,$school_id =null)
    {
     ...
     public function getItemAvailableBkp($item_id = null,$school_id =null) {
       /* $where = "";
