...
if (!function_exists('find_grade')) {
    ...
    function find_grade($grades, $percentage) {
            ...
            {   
                $grade->mark_from = (float)$grade->mark_from;
                $grade->mark_to = (float)$grade->mark_to;
...
if (!function_exists('numberToWords')) {
    function numberToWords2($number) {
        // Array of word equivalents for digits
        $words = array(
            0 => 'zero',
            1 => 'one',
            2 => 'two',
...
function numberToWords($number) {
    ...
    if ($number < 100) {
        ...
        $units = $number % 10;
        if ($units !== 0) {
            return ucwords($tens . '-' . $words[$units]);
        } else {
            return ucwords($tens);
        }
    }
    ...
    if ($number < 1000) {
        ...
        $tens_units = numberToWords($number % 100);
        if ($tens_units !== 'zero') {
            return ucwords($hundreds . ' and ' . $tens_units);
        } else {
            return ucwords($hundreds);
        }
    }

    if ($number >= 1000) {
        $thousands = numberToWords(floor($number / 1000)) . ' thousand';
        $hundreds = numberToWords($number % 1000);
        if ($hundreds !== 'zero') {
            return ucwords($thousands . ' ' . $hundreds);
        } else {
            return ucwords($thousands);
        }
    }
...
if (!function_exists('check_is_previous_year')) {

    function check_is_previous_year($school_id) {    
        ...
        $isPreviousYear =  $ci->accountledgers->get_single('financial_years',array('school_id'=>$school_id,'previous_financial_year_id'=>$financial_year->id));
        return !empty($isPreviousYear) ? 1 : 0;
    }
}
if (!function_exists('update_ledger_opening_balance')) {
    ...
    function update_ledger_opening_balance($ledger_ids,$school_id) {    
        ...
        $ledger_transactions=$ci->accountledgers->get_ledger_with_amount_list_with_balance($ledger_ids, $f_start,$f_end);
		// echo $ci->db->last_query();
		// die();
		 $ledgers = array();	
		 ...
		 $other_transactions=$ci->accountledgers->get_ledger_with_amount_list_with_balance_excluded($ledger_ids,null, $f_start,$f_end);
			// echo "<pre>";
			 //echo $ci->db->last_query();
        ...
        //    echo "<pre>"; var_dumsp($account_ledger_details); 
